...
- `data/played.json` — status de cada jogo (`{ "<id>": { "status": "played" | "finished", "dateAdded": "YYYY-MM-DD" } }`; ausência de entrada = não jogado). Marcações antigas no formato booleano (`{ "played": true }`), seja em `localStorage` de uma visita anterior ou coladas na caixa de importar, são lidas como `"finished"` — era o único significado que o booleano tinha.
...
- `game.html`, `game.js` — página de detalhe de um jogo (`game.html?id=<id>`), com capa maior, desenvolvedora/publicadora, datas por região, gênero/franquia/jogadores/classificação/serial (quando `data/details.json` tiver), sinopse e o controle de status (não jogado/jogado/finalizado, edita o mesmo estado da listagem).
- `common.js` — funções compartilhadas entre `app.js` e `game.js` (chave/leitura/escrita do `localStorage` de marcações, fallback de capa entre jsdelivr/thumbnails.libretro.com/placeholder, registro do service worker).
- `manifest.json`/`en/manifest.json`, `sw.js`, `assets/icon.svg` — suporte a PWA (item 9 do `ROADMAP.md`): permite instalar o catálogo como app e usar 100% offline depois da primeira visita. `sw.js` fica na raiz do site (escopo cobre `/en/` também) e é registrado por `common.js`; ver seção "Uso offline (PWA)" abaixo.
...
Na página, clique no ícone da coluna "Jogado" na linha do jogo (ou no botão equivalente na página de detalhe) para ciclar entre não jogado → jogado → finalizado → não jogado. Isso salva a marcação no `localStorage` do navegador imediatamente.
...
Alternativamente, edite `data/played.json` diretamente, adicionando o `id` do jogo (visível na URL ao passar o mouse ou no próprio arquivo `games.json`) com `{ "status": "played" }` ou `{ "status": "finished" }`.

## Uso offline (PWA)

O site é uma PWA (item 9 do `ROADMAP.md`): tem `manifest.json` e um service worker (`sw.js`) que faz cache de todos os arquivos estáticos e dos `data/*.json` na primeira visita. Depois disso:

- O navegador (Chrome/Edge) oferece a opção de "Instalar app", criando um ícone/atalho no desktop que abre o catálogo numa janela própria, sem barra de endereço.
- O site continua funcionando **sem internet e sem servidor local**, tanto instalado quanto numa aba normal do navegador — desde que ele já tenha sido aberto pelo menos uma vez com internet (pra registrar o service worker e popular o cache).
- Marcações de "jogado"/favoritos continuam no `localStorage`, isolado por navegador/instalação, igual já era antes da PWA.
- Service worker não funciona em `file://` — só em `http(s)://` (inclusive `localhost`), então testar essa parte localmente exige rodar o servidor da seção abaixo (ou usar o site já publicado no GitHub Pages).
- Ao atualizar `data/*.json` ou os arquivos estáticos, bump a constante `CACHE_NAME` em `sw.js` (ex. `lifebar-v2`) pra forçar a limpeza do cache antigo em quem já tinha instalado/visitado o site.
...
2. Rode `tools/publish_public.sh`: ele clona (se ainda não existir) `lifebar-game-catalog` em `~/Documents/development/lifebar-game-catalog` (ou em `$PUBLIC_REPO`, se definido), copia os arquivos públicos (`index.html`, `game.html`, `credits.html`, `en/`, `style.css`, `app.js`, `game.js`, `common.js`, `manifest.json`, `sw.js`, `assets/`, `data/`, `LICENSE`, `NOTICE.md`, `README.md`) e deixa tudo staged (`git add`) lá pra revisão — sem commitar nem dar push sozinho.
